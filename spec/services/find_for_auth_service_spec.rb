...
RSpec.describe FindForOauthService do
  ...
  context 'user has not authorization' do
    context 'user already exists' do
      let(:auth) { OmniAuth::AuthHash.new(provider: 'facebook', uid: 123_456, info: { email: user.email }) }
    ...
    context 'user does not exist' do
      let(:auth) { OmniAuth::AuthHash.new(provider: 'facebook', uid: 123_456, info: { email: 'new@user.com' }) }
